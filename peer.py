...
def upload_file(stub, file_path):
    for chunk_number, chunk in partition_file(file_path):
        ...
        response = stub.UploadFile(file_chunk)
    ...
    while True:
        ...
        if command == "upload":
            ...
            if os.path.exists(file_path):
                peer_channel = grpc.insecure_channel(f'localhost:{port}')
                peer_stub = p2p_pb2_grpc.P2PServiceStub(peer_channel)
                upload_file(peer_stub, file_path)
                register_peer(tracker_stub, f'localhost:{port}', [os.path.basename(file_path)])
